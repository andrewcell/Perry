...
public class MonsterDropCreator {
	...
	Data data = DataProviderFactory.Companion.getDataProvider(new File(System.getProperty("wzpath") + "/String.wz")).getData("MonsterBook.img");
	...
	for (Data dataz : data.getChildren()) {
		...
		sb.append("INSERT INTO ").append(monsterQueryData).append(" VALUES ");
		for (Data drop : dataz.getChildByPath("reward")) {
		    int itemid = DataTool.Companion.getInt(drop);
		    int rate = getChance(itemid, idtoLog, bossCache.containsKey(idtoLog));

		    for (Pair<Integer, MobInfo> Pair : mobCache) {
			if (Pair.getFirst() == monsterId) {
			    if (Pair.getSecond().getBoss() > 0) { // Is boss or not.
				if (rate <= 100000) {
				    if (Pair.getSecond().rateItemDropLevel() == 2) {
					rate *= 10;
				    } else if (Pair.getSecond().rateItemDropLevel() == 3) {
					switch (monsterId) {
					    case 8810018: // HT
						rate *= 48;
					    case 8800002: // Zakum
	...
	for (Pair<Integer, String> Pair : itemNameCache) {
	    if (Pair.getFirst() >= 2380000 && Pair.getFirst() <= lastmonstercardid) {
		bookName.append(Pair.getSecond());
		...
		for (Pair<Integer, MobInfo> Pair_ : mobCache) {
		    if (Pair_.getSecond().getName().equalsIgnoreCase(bookName.toString())) {
			int rate = 1000;
			if (Pair_.getSecond().getBoss() > 0) {
			    rate *= 25;
			}
			SQL.append("INSERT INTO ").append(monsterQueryData).append(" VALUES ");
			SQL.append("(DEFAULT, ");
			SQL.append(Pair_.getFirst()).append(", "); // Dropperid
			if (addFlagData) {
			    sb.append("'', "); // Flags
			}
			SQL.append(Pair.getFirst()).append(", "); // Itemid
			SQL.append("1, 1,"); // Item min and max
			SQL.append("0, "); // Quest
			SQL.append(rate);
			SQL.append(");\n");
                        SQL.append("-- Name : ").append(Pair.getSecond()).append("\n");
			break;
		    }
		}
        ...
        for (Pair<Integer, String> Pair : itemNameCache) {
            ...
            if (bookName.toString().contains(" Card"))
                ...
                if (Pair.getFirst() == lastmonsterbookid) continue; //Fix for multiple monster shet
                for (Pair<Integer, MobInfo> Pair_ : mobCache) {
                    if (Pair_.getSecond().getName().equalsIgnoreCase(bookName.toString())) {
                        ...
                        SQL.append(", ");
                        SQL.append(Pair_.getFirst()).append(");\n");
                        lastmonsterbookid = Pair.getFirst();
                        i++;
                        break;
                    }
                }
...
//	    case 1002905: // Targa hat [Malaysia boss]
	...
	switch (id / 10000) {
		...
		switch (id) {
		    case 2290096: // M. Hero 20
			return 800000; // 80% rate for HT
		    case 2290125: // M. Hero 30
			return 100000;
		}
		return 500;
...
//		    case 2070011: // M. Throwing star
	...
	IndiviualMonsterDrop.add(2290125); // M. Warrior 30
	...
	DataProvider data = DataProviderFactory.Companion.getDataProvider(new File(System.getProperty("wzpath") + "/String.wz"));
	...
	for (Data itemFolder : itemsData.getChildren()) {
	    ...
	    String itemName = DataTool.Companion.getString("name", itemFolder, "NO-NAME");
	...
	DataProvider data = DataProviderFactory.Companion.getDataProvider(new File(System.getProperty("wzpath") + "/String.wz"));
	DataProvider mobData = DataProviderFactory.Companion.getDataProvider(new File(System.getProperty("wzpath") + "/Mob.wz"));
	Data mob = data.getData("Mob.img");

	int id;
	Data monsterData;

	for (Data itemFolder : mob.getChildren()) { // Get the list of mobs from String.wz
	    id = Integer.parseInt(itemFolder.getName());

	    try {
		monsterData = mobData.getData(kotlin.text.StringsKt.padStart(id + ".img", 11, '0'));
		final int boss = id == 8810018 ? 1 : DataTool.Companion.getIntConvert("boss", monsterData.getChildByPath("info"), 0);
		...
		MobInfo mobInfo = new MobInfo(
			boss, // fix for HT
			DataTool.Companion.getIntConvert("rareItemDropLevel", monsterData.getChildByPath("info"), 0),
			DataTool.Companion.getString("name", itemFolder, "NO-NAME"));
